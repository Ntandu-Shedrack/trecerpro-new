...
export default function CategoryDialog({ 
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-slate-950 border border-slate-800 text-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-white">{editingCategory ? "Edit Category" : "New Category"}</DialogTitle>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
          <div className="grid gap-6 p-1">
            <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-800 space-y-4">
              ...
              <Field>
                <FieldLabel className="text-xs font-semibold uppercase tracking-wider text-slate-400">Category Name</FieldLabel>
                <FieldContent>
                  <Input 
                    placeholder="e.g., Servers, Networking, Workstations" 
                    {...form.register("name")} 
                    className={cn("h-10 bg-slate-900 border-slate-800 focus:bg-slate-950", form.formState.errors.name ? "border-destructive" : "")}
                  />
              ...
              <Field>
                <FieldLabel className="text-xs font-semibold uppercase tracking-wider text-slate-400">Description (Optional)</FieldLabel>
                <FieldContent>
                  <Textarea 
                    placeholder="What kind of assets belong to this category?" 
                    {...form.register("description")} 
                    className="bg-slate-900 border-slate-800 focus:bg-slate-950 min-h-[80px] resize-none"
                  />
                </FieldContent>
              </Field>
            </div>
              ...
              <div className="flex items-center justify-between px-1">
                <div className="space-y-0.5">
                  <h3 className="text-sm font-bold flex items-center gap-2 text-white">
                    Custom Attributes
                    <Badge variant="outline" className="h-5 px-1.5 text-[10px] font-bold border-slate-800">
                      {fields.length}
                    </Badge>
                  </h3>
                  <p className="text-xs text-slate-400">Define custom fields for assets in this category.</p>
                </div>
                <Button 
                  type="button" 
                  variant="outline" 
                  size="sm" 
                  className="gap-1.5 h-8 text-xs font-bold border-slate-850 hover:border-primary hover:text-primary transition-all bg-slate-900"
                ...
                <AnimatePresence initial={false}>
                  {fields.length === 0 ? (
                    <motion.div 
                      ...
                      className="flex flex-col items-center justify-center py-12 border border-dashed rounded-2xl bg-slate-900/20 border-slate-800 text-center gap-3"
                    >
                      <div className="h-10 w-10 rounded-full bg-slate-800 flex items-center justify-center">
                        <Plus className="h-5 w-5 text-slate-400" />
                      </div>
                      <div className="space-y-1">
                        <p className="text-sm font-medium text-slate-300">No custom fields yet</p>
                        <p className="text-xs text-slate-400">Add fields like IP Address, Serial Number, etc.</p>
                      </div>
                      <Button 
                        type="button" 
                        variant="secondary" 
                    ...
                    fields.map((field, index) => {
                      ...
                      return (
                        <motion.div 
                          ...
                          className="group relative flex flex-col p-4 rounded-2xl border border-slate-800 bg-slate-900/30 hover:border-primary/30 transition-all hover:shadow-md gap-3"
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-6 text-slate-500 cursor-grab active:cursor-grabbing">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 flex-1">
                              ...
                              <div className="md:col-span-4">
                                <Input 
                                  ...
                                  className="h-10 bg-slate-900 border-slate-800 focus:bg-slate-950 text-sm font-semibold"
                                />
                              </div>
                              ...
                              <div className="md:col-span-3">
                                <Input 
                                  ...
                                  className="h-10 bg-slate-900/40 border-slate-800 text-xs font-mono text-slate-400"
                                />
                              </div>
                              ...
                              <div className="md:col-span-3">
                                ...
                                >
                                  <SelectTrigger className="h-10 bg-slate-900 border-slate-850 focus:ring-1 focus:ring-primary shadow-none">
                                  ...
                                  <SelectContent className="bg-slate-950 border-slate-800 text-foreground">
                                    {Object.entries(TYPE_CONFIG).map(([key, config]) => (
                                      <SelectItem key={key} value={key}>
                                        <div className="flex items-center gap-2">
                                          <div className={cn("p-1 rounded shadow-sm border border-slate-800", config.bg)}>
                              ...
                              <div className="md:col-span-2 flex items-center justify-between md:justify-center gap-2">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  />
                                  <Label htmlFor={`required-${field.id}`} className="text-[9px] font-bold uppercase tracking-wider text-slate-400 cursor-pointer whitespace-nowrap">
                                    Req
                                  </Label>
                                </div>
                          ...
                          {attrType === "select" && (
                            <div className="pl-9 pr-2 space-y-1.5 animate-in slide-in-from-top-1 duration-200">
                              ...
                              <Input
                                ...
                                className="h-10 bg-slate-900 border-slate-800 text-xs font-semibold focus:bg-slate-950"
          ...
          <DialogFooter className="pt-6 gap-2 border-t border-slate-800 mt-4">
            <Button 
              type="button" 
              variant="ghost" 
              onClick={() => setIsDialogOpen(false)}
              disabled={isPending}
              className="font-bold text-xs uppercase tracking-wider text-slate-400 hover:text-white"
            >
              Cancel
            </Button>
            <Button 
              type="submit" 
              disabled={isPending}
              className="min-w-[140px] font-bold text-xs uppercase tracking-wider shadow-lg shadow-primary/20"
            >
              {isPending ? (
                <div className="flex items-center gap-2">
                  <div className="h-3 w-3 border-2 border-background border-t-transparent animate-spin rounded-full" />
                  Saving...
                </div>
